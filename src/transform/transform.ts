...
import { LanguageHeader, ParamInfo, paramInfo } from '../generator/common/helpers';
  ...
  for (const group of values(session.model.operationGroups)) {
    for (const op of values(group.operations)) {
      ...
      if (marshallingFormat !== 'na') {
        const bodyParam = values(op.parameters).where((each: Parameter) => { return each.protocol.http!.in === 'body'; }).first();
        if (bodyParam) {
  ...
  for (const group of values(session.model.operationGroups)) {
    for (const op of values(group.operations)) {
      ...
      for (const resp of values(op.responses)) {
        ...
        // fix up header response value types
